...
// Secret for HMAC (keep this safe!)
...
// Shared function to extract subject ID
...
// Shared function to extract detail path
...
// Drag token functions
function generateDragToken(path, ip, ua) {
  const expires = Date.now() + 60 * 1000; // 1 minute
...
// Add this helper for the RapidAPI button HTML
...
app.get('/movie', async (req, res) => {
  ...
  const path = req.baseUrl + req.path + (req.path === '/tv' 
    ? `?tmdbId=${req.query.tmdbId}&season=${req.query.season}&episode=${req.query.episode}` 
    : `?tmdbId=${req.query.tmdbId}`);

  const urlObj = new URL(req.protocol + '://' + req.get('host') + req.originalUrl);
  urlObj.searchParams.delete('drag_token');
  const cleanUrl = urlObj.pathname + (urlObj.search ? urlObj.search : '');

  if (!req.query.drag_token || !verifyDragToken(path, req.query.drag_token, ip, ua)) {
    const token = generateDragToken(path, ip, ua);
    const sep = cleanUrl.includes('?') ? '&' : '?';
    const redirectUrl = `${cleanUrl}${sep}drag_token=${token}`;
    return res.send(dragVerifyPage(redirectUrl));
  }
  ...
  try {
    ...
    const posterPath = tmdbResp.data.poster_path;
    ...
    if (!subjectId) {
      // Try fallback API before returning 404
      const fallbackData = await getFallbackMovieLinks(tmdbId);
      if (fallbackData && fallbackData.qualities && fallbackData.qualities.length) {
        let downloadsHtml = `<h2>Mp4 Downloads (Fallback)</h2><div class="buttons-grid">`;
        fallbackData.qualities.forEach((q, i) => {
          const mainLink = q.links.first || q.links.second || q.links.third || '';
          downloadsHtml += `
            <a href="#" class="btn download" data-url="${b64(mainLink)}" id="fb${i}">
              <span class="quality">${q.quality || 'Unknown'}</span>
        ...
        const pageHtml = `
          <html>
          <head>
            <title>Fallback Movie Downloads</title>
            <style>
              body {
              ...
              .container {
                ...
                min-width: 320px;
                max-width: 98vw;
                max-height: 90vh; /* Prevent container from overflowing */
          ...
          <body oncontextmenu="return false;">
            <div class="container">
              <h1>Fallback Movie Downloads</h1>
              ${downloadsHtml}
            </div>
            ${rapidApiButtonHtml()}
            <noscript>
            ...
            <script>
              // Anti-scrape: decode base64 URLs and redirect
              ...
              document.addEventListener('keydown', function(e) {
                if (
                  ...
                  throw new Error('Dev tools blocked');
                }
              });

              // Detect devtools open (resize-based & debugger-based)
              (function() {
                let threshold = 160;
      ...
      return res.status(404).send('<h2>Movie not Available Yet</h2>');
    }
    ...
    const downloadResp = await axios.get(`https://moviebox.ng/wefeed-h5-bff/web/subject/download?subjectId=${subjectId}&se=0&ep=0`, {
      headers: {
    ...
    let downloadsHtml = '';
    if (downloads.length) {
      downloadsHtml += `<h2>Mp4 Downloads (No Subtitles)</h2><div class="buttons-grid">`;
...
app.get('/tv', async (req, res) => {
  ...
  try {
    ...
    const subjectId = extractSubjectId(html, title);
    if (!subjectId) {
      // Try fallback TV API before returning 404
      ...
      if (fallbackData && fallbackData.results && fallbackData.results.length) {
        ...
        fallbackData.results.forEach((item, i) => {
          downloadsHtml += `
            <a href="#" class="btn download" data-url="${b64(item.download_url)}" id="fb${i}">
      ...
      // If fallback also fails, show 404
      return res.status(404).send('<h2>Series Not Available Yet</h2>');
    }
    ...
    const pageHtml = `
      ...
      <body oncontextmenu="return false;">
        <div class="container">
          <h1>${title} (${year})<br>Season ${season} Episode ${episode}</h1>
          ${downloadsHtml}
          ${captionsHtml}
          ${(!downloadsHtml && !captionsHtml) ? '<p>No downloads or captions available.</p>' : ''}
        </div>
