...
export default async function handler(req, res) {
  const { tmdbId, drag_token, header } = req.query;
  ...
  const path = '/api/movie?tmdbId=' + tmdbId;

  // Drag verify
  if (!drag_token || !verifyDragToken(path, drag_token, ip, ua)) {
    const token = generateDragToken(path, ip, ua);
    const sep = req.url.includes('?') ? '&' : '?';
    const redirectUrl = `/api/movie?tmdbId=${tmdbId}${sep}drag_token=${token}`;
  ...
  try {
    ...
    // if (!subjectId) {
      ...
      if (fallbackData && fallbackData.qualities && fallbackData.qualities.length) {
        downloadsHtml += `<h2>Mp4 Downloads (Fallback)</h2><div class="buttons-grid">`;
        fallbackData.qualities.forEach((q, i) => {
          const mainLink = q.links.first || q.links.second || q.links.third || '';
          downloadsHtml += `
            <a href="#" class="btn download" data-url="${b64(mainLink)}" id="fb${i}" data-ad="false">
            ...
            <script>
              document.querySelectorAll('.btn.download').forEach(btn => {
                ...
                btn.addEventListener('click', function(e) {
                  ...
                  if (!adVisited) {
                    adVisited = true;
                    btn.querySelector('.dl-label').textContent = 'Click again to Download';
                    btn.style.background = '#fbbf24';
                    btn.style.color = '#232323';
                    window.open('https://otieu.com/4/9583157', '_blank');
                    return;
                  }
